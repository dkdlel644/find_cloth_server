...
def build_graph(hub_module_url, target_image_path):
  ...
  # Copied a method of https://github.com/GoogleCloudPlat$form/cloudml-samples/blob/bf0680726/flowers/trainer/model.py#L181
  # and fixed for all type images (not only jpeg)
  def decode_and_resize(image_str_tensor):
    """Decodes jpeg string, resizes it and returns a uint8 tensor."""
    image = tf.image.decode_image(image_str_tensor, channels=CHANNELS)
    # Note resize expects a batch_size, but t$f_map supresses that index,
